...
[Überarbeitung_Ausarbeitung_Rechnen_mit_Fehlern.pdf](https://github.com/user-attachments/files/29740333/Uberarbeitung_Ausarbeitung_Rechnen_mit_Fehlern.pdf)
...
Der absolute Fehler $\Delta x:=|x- \Tilde{x}|$ beschreibt die Abweichung des gestörten Werts zum eigentlichen Wert. \\
\\
...
\begin{proof}
 Es gilt $\varepsilon_x = \frac{x- \Tilde{x}}{x} \Leftrightarrow \Tilde{x}=x \cdot(1- \varepsilon_x)$ und analog $\Tilde{y}=y \cdot(1-\varepsilon_y)$ \vspace{\baselineskip} \\
...
\begin{proof} 
    \begin{enumerate}[label=(\alph*)]
        ...
        \end{equation*}
        Es gilt $\frac{1}{x_n}(x_n-\sqrt{a})\leq 1$ und daraus folgt $x_{n+1}-\sqrt{a} \leq \frac{1}{2}(x_n-\sqrt{a})$ für $n \geq1$. Damit ist $\lim_{n \to \infty} x_n = \sqrt{a}$ bewiesen, da sich der Fehler mit jedem Schritt mindestens halbiert. Aus $x_n \geq 1$ folgt die Behauptung.
        \item Für $n \in \N$ gilt:
        \begin{equation*}
            x_n-\sqrt{a} \overset{\text{(a)}}{\leq} x_n- \frac{a}{x_n}= 2 \cdot x_n - 2 \cdot \left( \frac{1}{2} \cdot  \left( x_n + \frac{a}{x_n}\right) \right) = 2x_n-2x_{n+1} \overset{\text{(a)}}{\leq} 2(x_n- \sqrt{a}) 
        \end{equation*} 
    \end{enumerate}
\end{proof}
